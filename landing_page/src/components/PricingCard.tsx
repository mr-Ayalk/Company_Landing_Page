...
export const PricingCard: React.FC<PricingCardProps> = ({
    ...
    return (
        <div className={`
            ...
            ${isFeatured ? "border-[#FF6B00] scale-105 z-10" : "border-transparent hover:border-[#FF6B00] hover:scale-105 hover:z-10"}
            group
        `}>
            {/* ... badge code remains same ... */}

            <h3 className="text-[#8B5CF6] text-3xl font-bold mb-10 mt-4">{title}</h3>

            <ul className="flex-grow space-y-4 w-full mb-10">
                {features.map((feature, idx) => (
                    <li key={idx} className={`flex items-start gap-3 text-left ${!feature.included ? "opacity-40" : ""}`}>
                        {feature.included ? (
            ...
            <button className={`
                ...
                ${isFeatured ? "bg-[#FF6B00] text-white shadow-lg shadow-orange-200" : "bg-[#7C3AED] text-white group-hover:bg-[#FF6B00] group-hover:shadow-orange-200"}
            `}>
                {buttonText}
            </button>
        </div>
